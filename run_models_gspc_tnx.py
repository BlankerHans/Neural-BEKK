...
import matplotlib
matplotlib.use("Agg")  # headless: run as a plain script
import matplotlib.pyplot as plt
plt.style.use('ggplot')
# Silence the flood of "FigureCanvasAgg is non-interactive" warnings: we never
# display figures in this batch run, so make plt.show() a no-op.
...
# Robustness study: 2-asset (S&P 500 + 10Y yield) dataset prepared offline by
# scripts/prepare_gspc_tnx_data.py. Parameterized via env vars so the same
# script serves the full-history (gspc_tnx_1962) and modern post-Volcker
# (gspc_tnx_1990) cohorts. The split MUST match the prep/R split.
DATA_ID = os.environ.get("GSPC_DATA_ID", "gspc_tnx_1962")
GSPC_TRAIN_SIZE = float(os.environ.get("GSPC_TRAIN_SIZE", "0.60"))
GSPC_VAL_SIZE = float(os.environ.get("GSPC_VAL_SIZE", "0.15"))

AS_OF_DATE = pd.Timestamp(DATA_AS_OF)
YFINANCE_END = (AS_OF_DATE + pd.Timedelta(days=1)).strftime("%Y-%m-%d")

# asof_ prefix so evaluate_runs.py's discover_runs regex groups these as a
# separate as-of cohort ("asof_<DATA_ID>") next to the thesis runs.
RUN_ID = f"asof_{DATA_ID}_seed{SEED}"
...
# Load the offline-prepared 2-asset data instead of re-downloading, so the
# neural pipeline uses EXACTLY the same returns/splits the R BEKK/DCC models were
# fit on (scripts/prepare_gspc_tnx_data.py wrote close_/logret_/*_df_ CSVs).
close = pd.read_csv(data_dir / f"close_{DATA_ID}.csv", index_col=0, parse_dates=True)[macro_tickers]
logret = pd.read_csv(data_dir / f"logret_{DATA_ID}.csv", index_col=0, parse_dates=True)[macro_tickers]
print("Loaded:", data_dir / f"close_{DATA_ID}.csv", "->", close.shape)
print("Loaded:", data_dir / f"logret_{DATA_ID}.csv", "->", logret.shape)
...
train_df, val_df, test_df = train_val_test_split(input_df, train_size=GSPC_TRAIN_SIZE, val_size=GSPC_VAL_SIZE)
...
print(f"\n{'='*64}\n>>> [1/8] TRAINING lstm_normal (Gaussian LSTM)\n{'='*64}", flush=True)
...
w = torch.full((len(macro_tickers),), 1.0 / len(macro_tickers), dtype=torch.float32).to(device)  # gleichgewichtetes Portfolio
...
print(f"\n{'='*64}\n>>> [2/8] TRAINING lstm_student (Student-t LSTM)\n{'='*64}", flush=True)
...
print(f"\n{'='*64}\n>>> [3/8] TRAINING bekk_lstm_scalar (slow: BEKK kernel)\n{'='*64}", flush=True)
...
print(f"\n{'='*64}\n>>> [4/8] TRAINING bekk_lstm_vector (slow: BEKK kernel)\n{'='*64}", flush=True)
...
print(f"\n{'='*64}\n>>> [5/8] TRAINING bekk_lstm_mix (slowest: convex mixture)\n{'='*64}", flush=True)
...
print(f"\n{'='*64}\n>>> [6/8] TRAINING neural_bekk_asym_diag (Neural-BEKK)\n{'='*64}", flush=True)
...
print(f"\n{'='*64}\n>>> [7/8] TRAINING gru_normal (Gaussian GRU)\n{'='*64}", flush=True)
...
print(f"\n{'='*64}\n>>> [8/8] TRAINING gru_student (Student-t GRU)\n{'='*64}", flush=True)
...
print(f"\n{'='*64}\n>>> ALL 8 NEURAL MODELS DONE. Reading R BEKK/DCC + FHS + backtests\n{'='*64}", flush=True)
...
print(f"\n{'='*64}\n>>> DONE seed {SEED}: saved {result_save}\n{'='*64}", flush=True)
